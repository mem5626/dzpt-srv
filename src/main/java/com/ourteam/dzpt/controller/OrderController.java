...
public class OrderController {
    ...
    public Response createOrder(@RequestBody Map<String, Object> info) throws Exception{
        if(orderService.createOrder((Integer) info.get("listedGoodsId"),(Integer) info.get("buyer")) == 1)
    ...
    public Response cancelOrder(@RequestBody Integer orderId) throws Exception{
        if (orderId == null) throw new GlobalException(ExceptionMsg.ParameterError);
        if(orderService.cancelOrder(orderId)==1) return new Response(ExceptionMsg.Success);
    ...
    public Response affirmOrder(@RequestBody Map<String,Integer> info) throws Exception{
        if (info.get("orderId") == null || info.get("userId") == null) throw new GlobalException(ExceptionMsg.ParameterError);
        if(orderService.affirmOrder(info.get("orderId"),info.get("userId")) == 1) return new Response(ExceptionMsg.Success);
        else throw new GlobalException(ExceptionMsg.Error);
    }
