...
public class TradeBillController {
    ...
    public Response createTradeBill(HttpServletRequest request, @Validated @RequestBody TradeBill tradeBill,
        ...
        if(tradeBillService.createTradeBill(tradeBill)==1) return new Response(ExceptionMsg.Success);
    ...
    public Response getMyTrading(HttpServletRequest request,Integer userId) throws Exception{
        ...
        if (id != userId) throw new GlobalException(ExceptionMsg.NotAllow);
        HashMap<String,Object> map = new HashMap<>();
